...
def api_validate_payment(request):
...
def api_create_checkout_session(request):
    ...
    if gateway == 'stripe':
        ...
        session = stripe.checkout.Session.create(
            ...
            line_items = items,
            mode = 'payment',
            success_url = 'http://17.0.0.1/8000/cart/success/',
            cancel_url = 'http://17.0.0.1/8000/cart/'
        )
    ...
    if gateway == 'paypal':
        ...
        environment = SandboxEnvironment(client_id=(settings.PAYPAL_API_KEY_PUBLISHABLE, settings.PAYPAL_API_KEY_HIDDEN))
